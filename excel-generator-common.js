  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      function setCell(cell) {
          ...
          if (cell.border) {
            for (var i = cell.merge.to.row; i <= cell.merge.from.row; i++) {
              for (var j = cell.merge.to.col; j <= cell.merge.from.col; j++) {
                ...
                if (j == cell.merge.to.col) borderObj.left = cell.border.left;

                if (j == cell.merge.from.col)
                  borderObj.right = cell.border.right;

                if (i == cell.merge.to.row) borderObj.top = cell.border.top;
        ...
        if (cell.text) sheet.set(cell.col, cell.row, cell.text); //cell.text = string

        if (cell.font) sheet.font(cell.col, cell.row, cell.font); //cell.font = { name: 'Calibri', sz: '18', family: '3', scheme: '-', bold: 'true' }
        ...
        if (cell.fill) sheet.fill(cell.col, cell.row, cell.fill); //cell.fill = { type: 'solid', fgColor: 'BFBFBF' }
      ...
      if (typeof data.commonData !== 'undefined') {
        ...
        for (var i in cData) {
          ...
          for (var j in cData[i]) {
            ...
            if (typeof tmpdata.border !== 'undefined')
              coldata.border = tmpdata.border;
            if (typeof tmpdata.font !== 'undefined')
              coldata.font = tmpdata.font;
            if (typeof tmpdata.wrap !== 'undefined')
              coldata.wrap = tmpdata.wrap;
            if (typeof tmpdata.fill !== 'undefined')
              coldata.fill = tmpdata.fill;

            setCell(coldata);

            cno++;
          }
          if (typeof tmpdata.height !== 'undefined')
            sheet.height(srow, tmpdata.height);
          srow++;
        }
      }
      ...
      console.log(`Error in Excel Generator =>`, error.toString());
      reject(error);
    }
  });
};
