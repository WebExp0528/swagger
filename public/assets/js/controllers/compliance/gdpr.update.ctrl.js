(function () {
    ...
    GDPRAssessmentUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', '$filter', 'RiskService', 'ControlTestDataService', 'Utils'];
    app.controller('GDPRAssessmentUpdateCtrl', GDPRAssessmentUpdateController);

    function GDPRAssessmentUpdateController($scope, $rootScope, $state, $stateParams, $filter, RiskService, ControlTestDataService, Utils) {

        $scope.mainTitle = $state.current.title || 'loading';
        $scope.mainDesc = "Update GDPR Assessement";
        ...
        $scope.submitAction = function () {
            ...
            RiskService.FileUpload(idd, fileModel).then(function (res) {
                ...
                RiskService.UpdateGDPRAssessment($stateParams.id, $scope.VM).then(function (res) {
                    console.log('res', res);
                }).finally(function () {
                    $state.go('app.compliance.gdpr.main');
                });
            });
        };
        ...
        $scope.createAssessment = function(vendor){
            ...
            $state.go('app.compliance.gdpr.create', $scope.VM);
        };

        RiskService.GetGDPRAssessmentById($stateParams.id).then(function (data) {
            $scope.VM = data;
        ...
        $scope.addControlTestData = function() {	
        	$state.go('app.compliance.gdpr.addcontroltestdata', {pid: $stateParams.id});
        }
