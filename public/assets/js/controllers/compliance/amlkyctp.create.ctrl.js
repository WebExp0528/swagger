(function () {
    ...
    AMLKYCTPCreateController.$inject = ['$scope', '$rootScope', '$state', 'ComplianceService', 'Utils'];
    app.controller('AMLKYCTPCreateCtrl', AMLKYCTPCreateController);


    function AMLKYCTPCreateController($scope, $rootScope, $state, ComplianceService, Utils) {

        $scope.mainTitle = $state.current.title || 'loading';
        $scope.mainDesc = "Add AML/KYC Assessment";
        ...
        $scope.submitAction = function () {
            ...
            ComplianceService.PostAMLKYCTPAssessment($scope.VM).then(function (res) {
                if (res.status === 200) $state.go('app.compliance.amlkyctp.main');
            });
        };

        $scope.cancelAction = function () {
            if ($scope.Form.AmlKycTp.$dirty) {
