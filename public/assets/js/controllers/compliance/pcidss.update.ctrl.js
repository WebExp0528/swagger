(function () {
    ...
    PCIDSSUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', 'ComplianceService', 'Utils'];
    app.controller('PCIDSSUpdateCtrl', PCIDSSUpdateController);


    function PCIDSSUpdateController($scope, $rootScope, $state, $stateParams, ComplianceService, Utils) {
    	$scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Update PCIDSS Assessment";
        ...
        $scope.submitAction = function() {
            if($scope.Form.PciDss.$invalid) return false;
            ...
            ComplianceService.FileUpload(idd, fileModel).then(function(res){
                ...
                ComplianceService.UpdatePCIDSSAssessment($stateParams.id, $scope.VM).then(function (res) {
                    console.log('res',res);
                }).finally(function () {
                    $state.go('app.compliance.pcidss.main');
                });
            });
        };

        $scope.cancelAction = function() {
            if($scope.Form.PciDss.$dirty || $scope.isEdit) {
        ...
        ComplianceService.GetPCIDSSAssessment($stateParams.id).then(function(data){
            $scope.VM = data;
