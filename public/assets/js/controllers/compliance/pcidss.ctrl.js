(function () {
    ...
    PCIDSSController.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'ComplianceService', 'ChartFactory', 'Utils'];
    app.controller('PCIDSSCtrl', PCIDSSController);

    function PCIDSSController($scope, $rootScope, $state, $filter, $uibModal, ComplianceService, ChartFactory, Utils) {
        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Add Edit Search & Delete PCIDSS Assessments";
        ...
        $scope.downloadTemp = function () {
        	var dlTmpModal = $uibModal.open({
                ...
                resolve: {
                    items: function () {
                        return {
                            TempLoader: ComplianceService.GetPCIDSSTemplate(),
                            prefix: 'compliance/pcidss/templates/'
                        };
                    }
                }
            });
        ...
        $scope.downloadTemp2 = function () {
            ...
            var data = {};
            data.sheetName = "PCIDSS Test Plan";
            data.body = [];

            data.body.push({
                col: 3,
                row: 2,
                text: 'PCIDSS Test Plan Assessments',
        ...
        $scope.deleteAction = function (r) {
            ...
            confirmation.result.then(function () {
                ...
                ComplianceService.DeletePCIDSSAssessment(r.id).then(function (data) {
        ...
        function loadContents() {
            ComplianceService.GetPCIDSSStatus().then(function (data) {
                ChartFactory.CreatePieChart('By Status', 'By Status', data, 'rcsaStatus');
                return ComplianceService.GetPCIDSSPeriod();
            }).then(function (data) {
                ChartFactory.CreateMultiColChart('By period', data, 'periodChart');
                return ComplianceService.GetPCIDSSDept();
            }).then(function (data) {
        ...
        function loadAssessments() {
            ComplianceService.GetPCIDSSAssessments().then(function (data) {
