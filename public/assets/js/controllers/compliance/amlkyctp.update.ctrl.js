(function () {
    ...
    AMLKYCTestPlanUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', 'ComplianceService', 'Utils'];
    app.controller('AMLKYCTPUpdateCtrl', AMLKYCTestPlanUpdateController);


    function AMLKYCTestPlanUpdateController($scope, $rootScope, $state, $stateParams, ComplianceService, Utils) {
    	$scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Update AML/KYC Assessment";
        ...
        $scope.submitAction = function() {
            if($scope.Form.AmlKycTp.$invalid) return false;
            ...
            ComplianceService.FileUpload(idd, fileModel).then(function(res){
                ...
                ComplianceService.UpdateAMLKYCTPAssessment($stateParams.id, $scope.VM).then(function (res) {
                    console.log('res',res);
                }).finally(function () {
                    $state.go('app.compliance.amlkyctp.main');
                });
            });
        };

        $scope.cancelAction = function() {
            if($scope.Form.AmlKycTp.$dirty || $scope.isEdit) {
        ...
        ComplianceService.GetAMLKYCTPAssessment($stateParams.id).then(function(data){
            $scope.VM = data;
