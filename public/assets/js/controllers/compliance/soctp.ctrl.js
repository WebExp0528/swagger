(function () {
    ...
    SOCTestPlanController.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'ComplianceService', 'ChartFactory', 'Utils'];
    app.controller('SOCTPCtrl', SOCTestPlanController);

    function SOCTestPlanController($scope, $rootScope, $state, $filter, $uibModal, ComplianceService, ChartFactory, Utils) {
        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Add Edit Search & Delete SOC Assessments";
        ...
        $scope.downloadTemp = function () {
        	var dlTmpModal = $uibModal.open({
                ...
                resolve: {
                    items: function () {
                        return {
                            TempLoader: ComplianceService.GetSOCTPTemplate(),
                            prefix: 'compliance/soctp/templates/'
                        };
                    }
                }
            });
        ...
        $scope.deleteAction = function (r) {
            ...
            confirmation.result.then(function () {
                ...
                ComplianceService.DeleteSOCTPAssessment(r.id).then(function (data) {
        ...
        function loadContents() {
            ComplianceService.GetSOCTPStatus().then(function (data) {
                ChartFactory.CreatePieChart('By Status', 'By Status', data, 'rcsaStatus');
                return ComplianceService.GetSOCTPPeriod();
            }).then(function (data) {
                ChartFactory.CreateMultiColChart('By period', data, 'periodChart');
                return ComplianceService.GetSOCTPDept();
            }).then(function (data) {
        ...
        function loadAssessments() {
            ComplianceService.GetSOCTPAssessments().then(function (data) {
