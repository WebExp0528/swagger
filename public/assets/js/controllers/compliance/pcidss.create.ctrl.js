(function () {
    ...
    PCIDSSCreateController.$inject = ['$scope', '$rootScope', '$state', 'ComplianceService', 'Utils'];
    app.controller('PCIDSSCreateCtrl', PCIDSSCreateController);


    function PCIDSSCreateController($scope, $rootScope, $state, ComplianceService, Utils) {

        $scope.mainTitle = $state.current.title || 'loading';
        $scope.mainDesc = "Add PCIDSS Assessment";
        ...
        $scope.submitAction = function () {
            ...
            ComplianceService.PostPCIDSSAssessment($scope.VM).then(function (res) {
                if (res.status === 200) $state.go('app.compliance.pcidss.main');
            });
        };

        $scope.cancelAction = function () {
            if ($scope.Form.PciDss.$dirty) {
