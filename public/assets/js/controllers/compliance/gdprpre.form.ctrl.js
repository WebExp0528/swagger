(function () {
    ...
    GDPRAssessmentUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', '$filter', 'GDPRPreService', 'Utils'];
    app.controller('GDPRPreAssessmentFormCtrl', GDPRAssessmentUpdateController);

    function GDPRAssessmentUpdateController($scope, $rootScope, $state, $stateParams, $filter, GDPRPreService, Utils) {

        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "New GDPR Pre-Assessement";
        $scope.currPage = 'new';
        ...
        $scope.VM = {
            ...
            notApplicable: 0,
            inGoodCondition: 0,
            someAreasToImprove: 0,
            manyAreasToImprove: 0,
            needsASeriousLook: 0,
            completionDate: "",
            categoryData: []
        };
        ...
        $scope.submitAction = function () {
            ...
            for (var i = 0; i < $scope.VM.categoryData.length; i++) {
            	switch($scope.VM.categoryData[i].response) {
	        		case "Fully Achieved":
	        			$scope.VM.inGoodCondition++;
	        			break;
	        		case "Largely Achieved":
	        			$scope.VM.someAreasToImprove++;
	        			break;
	        		case "Partially Achieved":
	        			$scope.VM.manyAreasToImprove++;
	        			break;
	        		case "Not Achieved":
	        			$scope.VM.needsASeriousLook++;
	        			break;
	        		case "Skipped":
	        		case "Not Applicable":
	        			break;
        	...
        	GDPRPreService.Post($scope.VM).then(function (res) {
                console.log('res', res);
            }).finally(function () {
        ...
        function populateControlData(data) {
        	if (typeof $scope.VM !== 'undefined') {
        ...
        $scope.reviewAssessment = function() {
	        ...
	        console.log("categoryData: ", $scope.VM.categoryData);
