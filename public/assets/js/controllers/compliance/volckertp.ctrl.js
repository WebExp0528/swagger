(function () {
    ...
    VOLCKERTestPlanController.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'ComplianceService', 'ChartFactory', 'Utils'];
    app.controller('VOLCKERTPCtrl', VOLCKERTestPlanController);

    function VOLCKERTestPlanController($scope, $rootScope, $state, $filter, $uibModal, ComplianceService, ChartFactory, Utils) {
        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Add Edit Search & Delete Volcker Assessments";
        ...
        $scope.downloadTemp = function () {
        	var dlTmpModal = $uibModal.open({
                ...
                resolve: {
                    items: function () {
                        return {
                            TempLoader: ComplianceService.GetVOLCKERTPTemplate(),
                            prefix: 'compliance/volckertp/templates/'
                        };
                    }
                }
            });
        ...
        $scope.downloadTemp2 = function () {
            ...
            var data = {};
            data.sheetName = "Volcker Test Plan";
            data.body = [];

            data.body.push({
                col: 3,
                row: 2,
                text: 'Volcker Test Plan Assessments',
        ...
        $scope.deleteAction = function (r) {
            ...
            confirmation.result.then(function () {
                ...
                ComplianceService.DeleteVOLCKERTPAssessment(r.id).then(function (data) {
        ...
        function loadContents() {
            ComplianceService.GetVOLCKERTPStatus().then(function (data) {
                ChartFactory.CreatePieChart('By Status', 'By Status', data, 'rcsaStatus');
                return ComplianceService.GetVOLCKERTPPeriod();
            }).then(function (data) {
                ChartFactory.CreateMultiColChart('By period', data, 'periodChart');
                return ComplianceService.GetVOLCKERTPDept();
            }).then(function (data) {
        ...
        function loadAssessments() {
            ComplianceService.GetVOLCKERTPAssessments().then(function (data) {
