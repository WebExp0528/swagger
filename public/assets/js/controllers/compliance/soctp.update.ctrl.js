(function () {
    ...
    SOCTestPlanUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', 'ComplianceService', 'Utils'];
    app.controller('SOCTPUpdateCtrl', SOCTestPlanUpdateController);


    function SOCTestPlanUpdateController($scope, $rootScope, $state, $stateParams, ComplianceService, Utils) {
    	$scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Update SOC Assessment";
        ...
        $scope.submitAction = function() {
            if($scope.Form.SocTp.$invalid) return false;
            ...
            ComplianceService.FileUpload(idd, fileModel).then(function(res){
                ...
                ComplianceService.UpdateSOCTPAssessment($stateParams.id, $scope.VM).then(function (res) {
                    console.log('res',res);
                }).finally(function () {
                    $state.go('app.compliance.soctp.main');
                });
            });
        };

        $scope.cancelAction = function() {
            if($scope.Form.SocTp.$dirty || $scope.isEdit) {
                var confirm = Utils.CreateConfirmModal("Confirmation", "Do you want to cancel and if yes you should go back to previous screen", "Yes", "No");
                confirm.result.then(function(){ $state.go('app.compliance.soctp.main'); });
                return false;
            }
            $state.go('app.compliance.soctp.main');
        };
