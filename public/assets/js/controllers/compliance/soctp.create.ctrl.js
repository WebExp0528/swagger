(function () {
    ...
    SOCTPCreateController.$inject = ['$scope', '$rootScope', '$state', 'ComplianceService', 'Utils'];
    app.controller('SOCTPCreateCtrl', SOCTPCreateController);


    function SOCTPCreateController($scope, $rootScope, $state, ComplianceService, Utils) {

        $scope.mainTitle = $state.current.title || 'loading';
        $scope.mainDesc = "Create a SOC Test Plan";
        ...
        $scope.submitAction = function () {
            ...
            ComplianceService.PostSOCTPAssessment($scope.VM).then(function (res) {
                if (res.status === 200) $state.go('app.compliance.soctp.main');
            });
        };

        $scope.cancelAction = function () {
            if ($scope.Form.SocTp.$dirty) {
