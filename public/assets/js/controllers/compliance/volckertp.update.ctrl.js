(function () {
    ...
    VOLCKERTestPlanUpdateController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', 'ComplianceService', 'Utils'];
    app.controller('VOLCKERTPUpdateCtrl', VOLCKERTestPlanUpdateController);


    function VOLCKERTestPlanUpdateController($scope, $rootScope, $state, $stateParams, ComplianceService, Utils) {
    	$scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Update Volcker Assessment";
        ...
        $scope.submitAction = function() {
            if($scope.Form.VolckerTp.$invalid) return false;
            ...
            ComplianceService.FileUpload(idd, fileModel).then(function(res){
                ...
                ComplianceService.UpdateVOLCKERTPAssessment($stateParams.id, $scope.VM).then(function (res) {
                    console.log('res',res);
                }).finally(function () {
        ...
        $scope.cancelAction = function() {
            if($scope.Form.VolckerTp.$dirty || $scope.isEdit) {
                var confirm = Utils.CreateConfirmModal("Confirmation", "Do you want to cancel and if yes you should go back to previous screen", "Yes", "No");
                confirm.result.then(function(){ $state.go('app.compliance.volckertp.main'); });
                return false;
            }
            $state.go('app.compliance.volckertp.main');
        };

        ComplianceService.GetVOLCKERTPAssessment($stateParams.id).then(function(data){
            $scope.VM = data;
