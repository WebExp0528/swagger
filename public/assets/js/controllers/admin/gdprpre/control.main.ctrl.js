(function () {
    ...
    ControlMgmCtrl.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'GDPRPreService', 'Utils'];
    app.controller('ControlMgmCtrl', ControlMgmCtrl);

    function ControlMgmCtrl($scope, $rootScope, $state, $filter, $uibModal, GDPRPreService, Utils) {
        var vm = this;
        vm.mainTitle = $state.current.title;
        vm.mainDesc = "GDPR Pre-Assessment Control Management";
        ...
        GDPRPreService.GetControl().then(function (data) {
            ...
            angular.forEach(data, function (obj, key) {
                vm.Grid1.Data.push({
                    ...
                    controlStatement: obj.controlStatement,
        ...
        vm.deleteAction = function (r) {
            ...
            confirmation.result.then(function () {
                ...
                var rowId = r.id;
                GDPRPreService.DeleteControl(rowId).then(function (data) {
                    if (data.status === 200){
                        vm.Grid1.Total--;
