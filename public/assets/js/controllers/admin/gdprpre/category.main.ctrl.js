(function () {
    ...
    CategoryMgmCtrl.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'GDPRPreService', 'Utils'];
    app.controller('CategoryMgmCtrl', CategoryMgmCtrl);

    function CategoryMgmCtrl($scope, $rootScope, $state, $filter, $uibModal, GDPRPreService, Utils) {
        var vm = this;
        vm.mainTitle = $state.current.title;
        vm.mainDesc = "GDPR Pre-Assessment Category Management";
        ...
        vm.deleteAction = function (r) {
            ...
            confirmation.result.then(function () {
                ...
                var rowId = r.id;
                GDPRPreService.DeleteCategory(rowId).then(function (data) {
                    if (data.status === 200){
                        vm.Grid1.Total--;
