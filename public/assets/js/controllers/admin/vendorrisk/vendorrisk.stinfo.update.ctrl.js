(function () {
    ...
    app.controller('VendorriskAdminStinfoUpdateCtrl', VendorStinfoUpdateController);
    function VendorStinfoUpdateController($scope, $rootScope, $state, VendorService, Utils, $filter, $location, UniqueID) {
        ...
        $scope.submitAction = function () {
            ...
            $scope.VM.assessmentDtStr = $scope.VM.assessmentsDate;
            ...
            VendorService.UpdateRam($state.params.id, $scope.VM).then(function (res) {
                if (res.status === 200) {
                    ...
                    $state.go('app.admin.vendorrisk.main');
                }
            });
        };
        ...
        $scope.calculateRiskScore = function(vendor) {
        ...
        function calculateRiskScore(vendor){
            VendorService.calcMetrics($state.params.id, vendor.vendorName).then(function () {
            });
        }
    }
