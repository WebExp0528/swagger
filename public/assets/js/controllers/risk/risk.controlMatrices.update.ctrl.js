(function () {
  ...
  RiskControlMatricesFormController.$inject = [
    '$scope',
    '$rootScope',
    '$state',
    '$stateParams',
  ...
  app.controller(
    ...
    RiskControlMatricesFormController
  );

  function RiskControlMatricesFormController(
    $scope,
    $rootScope,
    $state,
    $stateParams,
    ...
    $scope.mainDesc = 'Update Risk Ccontrol Matrix';
    ...
    $scope.submitAction = function () {
      ...
      ComplianceService.UpdateSOXRCMAssessment($stateParams.id, $scope.VM).then(
        function (res) {
          if (res.status === 200) {
            var fileModel = $scope.VM.filemodel;
            ComplianceService.FileUpload($stateParams.id, fileModel)
              .then(function (res) {
                console.log(res);
              })
              .finally(function () {
      ...
      $scope.VM.key = idd;
      ComplianceService.FileUpload(idd, fileModel)
        .then(function (res) {
          if (res.status === 200) {
        ...
        .finally(function () {
          ComplianceService.UpdateSOXRCMAssessment($stateParams.id, $scope.VM)
            .then(function (res) {
              console.log('res', res);
            })
            .finally(function () {
    ...
    ComplianceService.GetSOXRCMAssessment($stateParams.id).then(function (
      data
    ) {
      $scope.VM = data;
    ...
    $scope.downloadExcel = function () {
      ...
      var inputdata = [
        [
          'Name',
          angular.isUndefined($scope.VM.assessName)
            ? ''
            : $scope.VM.assessName + '',
          '',
          'Description',
          angular.isUndefined($scope.VM.assessDesc)
            ? ''
            : $scope.VM.assessDesc + '',
        ],
        [
          ...
          '',
          'Department',
          angular.isUndefined($scope.VM.business)
            ? ''
            : $scope.VM.business + '',
        ],
        [
          'Frequency',
          angular.isUndefined($scope.VM.frequency)
            ? ''
            : $scope.VM.frequency + '',
          '',
          'Status',
          angular.isUndefined($scope.VM.approval)
            ? ''
            : $scope.VM.approval + '',
        ],
        [
          'Priority',
          angular.isUndefined($scope.VM.priority)
            ? ''
            : $scope.VM.priority + '',
          '',
          'Responsible Person',
          angular.isUndefined($scope.VM.resPerson)
            ? ''
            : $scope.VM.resPerson + '',
        ] /*,
                 [
      ...
      ComplianceService.DownloadExcel(data)
