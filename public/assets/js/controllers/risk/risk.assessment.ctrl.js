(function () {
    ...
    RiskAssessmentCtrl.$inject = ['$scope', '$rootScope', '$state', '$filter', '$uibModal', 'RiskService', 'OPRiskService', 'ChartFactory', 'Utils'];
    app.controller('RiskAssessmentCtrl', RiskAssessmentCtrl);

    function RiskAssessmentCtrl($scope, $rootScope, $state, $filter, $uibModal, RiskService, OPRiskService, ChartFactory, Utils) {
        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "Add Edit Search & Delete Risk Control Self Assessments";
        ...
        RiskService.GetStatus()
            .then(function (data) {
                ...
                ChartFactory.CreatePieChart('Risk Type Severity', 'Risk Type Severity', data, 'Status');
                /*return RiskService.GetPeriod();
            })
            .then(function (data) {
                ChartFactory.CreateMultiColChart('By Period', data, 'periodChart');*/
                return RiskService.GetDept();
            })
            .then(function (data) {
        ...
        function loadAssessments() {
            RiskService.GetAssessment().then(function (data) {
