(function(){
    ...
    FindingsController.$inject = ['$scope','$rootScope','$state', '$filter', 'MitigateService', 'AuditService', 'ChartFactory', 'Utils'];
    app.controller('FindingsCtrl', FindingsController);

    function FindingsController ($scope, $rootScope, $state, $filter, MitigateService, AuditService, ChartFactory, Utils){
        $scope.mainTitle = $state.current.title;
        $scope.mainDesc = "View Findings";
        ...
        MitigateService.GetStatus().then(function(data){
            ChartFactory.CreatePieChart('Findings By Status', 'Findings By Status', data, 'statusChart');
            // setupStatusChart(status);
            return MitigateService.GetPeriod();
        }).then(function(data){
            ChartFactory.CreateMultiColChart('Findings By Period', data, 'periodChart');
            ...
            ChartFactory.CreatePieChart('Findings By Severity', 'Findings By Severity', datalist, 'severityChart');
        ...
        function loadFindings() {
            AuditService.GetFindings().then(function(data) {
                data.forEach(function(r, i){
                    r.remediationDate = Utils.createDate(r.remediation_date);
                    r.identifiedDate = Utils.createDate(r.identified_date);
                    AuditService.GetEachAudit(r.auditId).then(function(auditData) {
                        r.auditData = auditData;
                    });
                    AuditService.GetEachTopic(r.topicId).then(function(topicData) {
                        r.topicData = topicData;
                    });
                });
