(function(){
    ...
    DashboardController.$inject = ['$scope','$rootScope','$state', '$filter', 'DashService' , 'calendarConfig', 'ChartFactory', 'Utils'];
    app.controller('DashboardCtrl', DashboardController);

    function DashboardController ($scope, $rootScope, $state, $filter, DashboardService, calendarConfig, ChartFactory, Utils){
        ...
        DashboardService.GetDashboard().then(function(data){
            ...
            for(var i in data){
                ...
                var d1 = moment(tp.createdDate);
                var d2 = moment(tp.dueDate);
                tp.createdDate = (d1.isValid()) ? d1.format(dtype) : '';
                tp.dueDate = (d2.isValid()) ? d2.format(dtype) : '';
            }
            ...
            for(var i in data){
                ...
                var dtype = 'MM-DD-YYYY';
                var d1 = moment(tp.alertCreatedDate);
                var d2 = moment(tp.alertDueDate);
                tp.alertCreatedDate = (d1.isValid()) ? d1.format(dtype) : '';
                tp.alertDueDate = (d2.isValid()) ? d2.format(dtype) : '';
            }
        ...
        $scope.update = function (a) {
        	console.log(a.id);
        	if (a.activityType === 'RCSA') {
        		$state.go('app.dashboard.rcsa.update', {id: a.id});
        	} else if (a.activityType === 'PRC') {
        		////
        	} else if (a.activityType === 'IT Risk Assessments') {
        		$state.go('app.dashboard.itriskassessment.update', {id: a.id});
        	} else if (a.activityType === 'Audit Assessments') {
        		////
        	} else if (a.activityType === 'SOXTP') {
        		$state.go('app.dashboard.soxtp.update', {id: a.id});
        	}  else if (a.activityType === 'SOXRCM') {
        		$state.go('app.dashboard.soxrcm.update', {id: a.id});
        	}  else if (a.activityType === 'SOXPRA') {
        		$state.go('app.dashboard.soxpra.update', {id: a.id});
        	}
        }
