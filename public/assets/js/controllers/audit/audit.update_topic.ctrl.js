(function () {
  AuditUpdateTopicController.$inject = [
    '$scope',
    '$stateParams',
    '$rootScope',
    '$state',
  ...
  function AuditUpdateTopicController(
    $scope,
    $stateParams,
    $rootScope,
    $state,
    ...
    AuditService.GetEachTopic(topic_id)
      ...
      .then(function (data) {
        ...
        return AuditService.GetTaskByTopic(topic_id);
      })
      .then(function (data) {
    ...
    vm.submitAction = function () {
      ...
      AuditService.FileUpload(idd, fileModel)
        ...
        .finally(function () {
          AuditService.UpdateTopic(topic_id, vm.formdata)
            .then(function (res) {})
            .finally(function () {
              $rootScope.app.Mask = false;
    ...
    vm.deleteAction = function (r) {
      ...
      confirmation.result.then(function () {
        $rootScope.app.Mask = true;
        AuditService.DeleteTask(r.id).then(function (data) {
          if (data.status === 200) {
            AuditService.GetEachTopic(topic_id)
              .then(function (res) {
                audit_id = res.auditId;
