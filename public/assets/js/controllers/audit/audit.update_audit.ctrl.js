(function () {
    AuditUpdateController.$inject = ['$scope', '$rootScope', '$state', '$uibModal', '$filter', 'AuditService', 'Utils', 'RiskService'];
    app.controller('AuditUpdate_AuditCtrl', AuditUpdateController);

    function AuditUpdateController($scope, $rootScope, $state, $uibModal, $filter, AuditService, Utils, RiskService) {
        var vm = this;
        ...
        AuditService.GetEachAudit(audit_id).then(function (res) {
            ...
            vm.formdata = res;
            vm.formdata.riskProfileModel = vm.formdata.riskProfileModel == null ? [] : vm.formdata.riskProfileModel;
            vm.formdata.controlTestPlanModel = vm.formdata.controlTestPlanModel == null ? [] : vm.formdata.controlTestPlanModel;
        ...
        $scope.addRiskProfiles = function() {
            ...
            RiskService.GetRiskProfile().then(function (data) {
                ...
                profileModal.result.then(function (list) {
                    $scope.isEdit = true;
                    $scope.vm.formdata.riskProfileModel = $scope.vm.formdata.riskProfileModel.concat(list);
        ...
        $scope.addControlTestPlans = function () {
            ...
            RiskService.GetControlTestPlan().then(function (data) {
                ...
                controlModal.result.then(function (list) {
                    $scope.isEdit = true;
                    $scope.vm.formdata.controlTestPlanModel = $scope.vm.formdata.controlTestPlanModel.concat(list);
                    $rootScope.controlTestPlanModel = $scope.vm.formdata.controlTestPlanModel;
                });
                $rootScope.app.Mask = false;
            });
        };
        ...
        $scope.editStakeholders = function() {
            ...
            var stakeholderModal = Utils.CreateSelectListView("Select Stakeholders", stakeholdersData, headers, cols);
            stakeholderModal.result.then(function (list) {
                $scope.isEdit = true;
                if ($scope.vm.formdata.stakeholdersVM == null) {
                    $scope.vm.formdata.stakeholdersVM = [];
                }
                $scope.vm.formdata.stakeholdersVM = $scope.vm.formdata.stakeholdersVM.concat(list);
            });
            $rootScope.app.Mask = false;
        };
