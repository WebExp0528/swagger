(function () {
    AuditUpdateActionController.$inject = ['$scope', '$rootScope', '$state', '$stateParams', '$uibModal', '$filter', 'AuditService', 'Utils'];
    app.controller('AuditUpdate_ActionCtrl', AuditUpdateActionController);

    function AuditUpdateActionController($scope, $rootScope, $state, $stateParams, $uibModal, $filter, AuditService, Utils) {
        var vm = this;
        ...
        vm.submitAction = function(){
            ...
            AuditService.FileUpload(idd, fileModel).then(function(res){
                ...
                AuditService.UpdateAction(action_id, vm.formdata).then(function (res) {
                }).finally(function () {
                    $rootScope.app.Mask = false;
