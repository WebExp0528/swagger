(function () {
	AssetInventoryUpdateController.$inject = ['$scope', '$rootScope', '$state', '$uibModal', '$filter', 'DataInventoryService', 'Utils'];
    app.controller('AssetInventoryUpdateCtrl', AssetInventoryUpdateController);

    function AssetInventoryUpdateController($scope, $rootScope, $state, $uibModal, $filter, DataInventoryService, Utils) {
        var vm = this;
        vm.mainTitle = $state.current.title;
        vm.mainDesc = "Update Asset";
        var assetId = $state.params.assetId;

        DataInventoryService.GetOneAsset(assetId).then(function (data) {
            vm.formdata = data;
        });
        ...
        vm.submitAction = function(){
            ...
            DataInventoryService.PutAsset(assetId, vm.formdata).then(function (res) {
        ...
        vm.downloadExcel = function () {
            ...
            var wval = [10, 20, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15];
            data.widths = [];
