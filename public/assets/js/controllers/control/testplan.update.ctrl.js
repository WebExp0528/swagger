(function () {
  TestPlanUpdateController.$inject = [
    '$scope',
    '$rootScope',
    '$state',
    '$stateParams',
  ...
  function TestPlanUpdateController(
    $scope,
    $rootScope,
    $state,
    $stateParams,
    ...
    $scope.addControls = function () {
      ...
      OPRiskService.GetControlData().then(function (data) {
        data.forEach(function (c, i) {
          ...
          var tmpRow = $filter('filter')($scope.VM.controlDataModel, {
            id: c.id,
          });
          if (tmpRow.length > 0) {
            c.Selected = true;
    ...
    $scope.submitAction = function () {
      ...
      ControlService.FileUpload(idd, fileModel)
        ...
        .finally(function () {
          ControlService.UpdateTestPlans($stateParams.id, $scope.VM)
            .then(function (res) {
              console.log('res', res);
            })
            .finally(function () {
